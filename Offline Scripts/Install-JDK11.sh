...
jdkTargz="/opt/openjdk-11+28_linux-x64_bin.tar.gz"
...
echo "— — 存在jdk压缩包 — —"
	echo "正在解压jdk压缩包..."
	tar -zxvf /opt/openjdk-11+28_linux-x64_bin.tar.gz -C /opt
	...
	mv /opt/jdk-11 /opt/install/java/java11
	# 检查配置信息
	checkExist	
	echo "---------------------------------"
	echo "正在配置jdk环境..."
	sed -i '$a export JAVA_HOME=/opt/install/java/java11' /etc/profile
